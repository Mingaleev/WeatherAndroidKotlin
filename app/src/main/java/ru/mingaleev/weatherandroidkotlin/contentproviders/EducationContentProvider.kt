package ru.mingaleev.weatherandroidkotlin.contentproviders
...
import ru.mingaleev.weatherandroidkotlin.MyApp.Companion.getWeatherDatabase
import ru.mingaleev.weatherandroidkotlin.R
import ru.mingaleev.weatherandroidkotlin.model.room.WeatherEntity
import ru.mingaleev.weatherandroidkotlin.utils.CITY
import ru.mingaleev.weatherandroidkotlin.utils.ID
import ru.mingaleev.weatherandroidkotlin.utils.TEMPERATURE


private const val URI_ALL = 1 // URI для всех записей
private const val URI_ID = 2 // URI для конкретной записи
private const val ENTITY_PATH = ".model.room.WeatherEntity"
...
class EducationContentProvider : ContentProvider() {
    ...
    private fun mapperRawToWeatherEntity(values: ContentValues?): WeatherEntity {
        return if (values == null) {
            ...
            val name = values[CITY] as String
            val temperature = values[TEMPERATURE] as Int
            WeatherEntity(id = id, nameCity = name, temperature = temperature)
        }
    }
}